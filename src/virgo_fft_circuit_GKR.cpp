...
{
    C_struct.circuit_val.push_back(new prime_field::field_element[1]);
    C_struct.circuit_val[0][0] = prime_field::field_element(1); //input layer
    C_struct.size.push_back(1);
    for(int i = 0; i < lg_size; ++i)
    {
        C_struct.circuit_val.push_back(new prime_field::field_element[1 << (i + 1)]);
        for(int j = 0; j < (1 << i); ++j)
        {
            C_struct.circuit_val[i + 1][j << 1] = C_struct.circuit_val[i][j] * r[i];
            C_struct.circuit_val[i + 1][j << 1 | 1] = C_struct.circuit_val[i][j] * (prime_field::field_element(1) - r[i]);
        }
        C_struct.size.push_back(1 << (i + 1));
    }
    //ifft
    ...
    for(int dep = lg_size - 1; dep >= 0; --dep)
    {
        ...
        int pre = cur - 1;
        C_struct.circuit_val.push_back(new prime_field::field_element[1 << lg_size]);
        C_struct.size.push_back(1 << lg_size);
        prime_field::field_element x = prime_field::field_element(1);
        {
            ...
            for(int k = 0; k < blk_size / 2; ++k)
            {
                ...
                {
                    auto l_value = C_struct.circuit_val[pre][double_k << (dep + 1) | j], r_value = x_arr[k] * C_struct.circuit_val[pre][double_k << (dep + 1) | (1 << dep) | j];
                    C_struct.circuit_val[cur][k << dep | j] = l_value + r_value;
                    C_struct.circuit_val[cur][(k + blk_size / 2) << dep | j] = l_value - r_value;
                }
            }
        }
    }
    int last_layer_id = C_struct.circuit_val.size();
    C_struct.circuit_val.push_back(new prime_field::field_element[1 << lg_size]);
    C_struct.size.push_back(1 << lg_size);
    for(int i = 0; i < (1 << lg_size); ++i)
    {
        C_struct.circuit_val[last_layer_id][i] = C_struct.circuit_val[last_layer_id - 1][i] * inv_n;
    }
    ...
    last_layer_id = C_struct.circuit_val.size();
    C_struct.circuit_val.push_back(new prime_field::field_element[64 * (1 << lg_size)]);
    C_struct.size.push_back(64 << lg_size);
    ...
    for(int i = 0; i < 64; ++i)
    {
        ...
        {
            C_struct.circuit_val[last_layer_id][j + (i << lg_size)] = C_struct.circuit_val[last_layer_id - 1][j] * x;
            x = x * eval_point;
        }
    }
    last_layer_id = C_struct.circuit_val.size();
    C_struct.circuit_val.push_back(new prime_field::field_element[64]);
    C_struct.size.push_back(64);
    for(int i = 0; i < 64; ++i)
    {
        C_struct.circuit_val[last_layer_id][i] = prime_field::field_element(0);
        for(int j = 0; j < (1 << lg_size); ++j)
        {
            C_struct.circuit_val[last_layer_id][i] = C_struct.circuit_val[last_layer_id][i] + C_struct.circuit_val[last_layer_id - 1][j + i * (1 << lg_size)];
        }
    }
}
...
linear_poly2 *add_mult_sum, *V_mult_add, *addV_array;

void init_array(int max_bit_length)
{
    add_mult_sum = new linear_poly2[(1 << max_bit_length)];
    V_mult_add = new linear_poly2[(1 << max_bit_length)];
    addV_array = new linear_poly2[(1 << max_bit_length)];
...
quadratic_poly2 sumcheck_phase1_update(prime_field::field_element previous_random, int current_bit, int total_uv)
{
    quadratic_poly2 ret = quadratic_poly2(prime_field::field_element(0), prime_field::field_element(0), prime_field::field_element(0));
...
quadratic_poly2 sumcheck_phase2_update(prime_field::field_element previous_random, int current_bit, int total_uv)
{
    quadratic_poly2 ret = quadratic_poly2(prime_field::field_element(0), prime_field::field_element(0), prime_field::field_element(0));
    for(int i = 0; i < (total_uv >> 1); ++i)
    {
...
bool addition_layer(prime_field::field_element &alpha_beta_sum, prime_field::field_element *c_val, int size_poly, int num_poly, prime_field::field_element *r_0, prime_field::field_element *one_minus_r_0, prime_field::field_element *r_1, prime_field::field_element *one_minus_r_1, prime_field::field_element *r_u, prime_field::field_element *one_minus_r_u, prime_field::field_element *r_v, prime_field::field_element *one_minus_r_v)
{
    ...
    {
        quadratic_poly2 poly = sumcheck_phase1_update(previous_random, i, (1 << (log_uv - i)));
        proof_size += sizeof(quadratic_poly2);
        previous_random = r_u[i];
        if(poly.eval2(0) + poly.eval2(1) != alpha_beta_sum)
        {
            return 0;
        }
        alpha_beta_sum = poly.eval2(r_u[i]);
    }

    auto v_u = V_mult_add[0].eval2(previous_random);
 ...
 */
        ...
        int pre = cur - 1;
        {
            ...
            for(int i = 0; i < total_uv; ++i)
            {
                V_mult_add[i] = C_struct.circuit_val[pre][i];
            ...
            for(int k = 0; k < blk_size / 2; ++k)
            {
                for(int j = 0; j < (1 << dep); ++j)
                {
                    ...
                    addV_array[u_id].b = addV_array[u_id].b + tmp * x * C_struct.circuit_val[pre][v_id];
            ...
            for(int i = 0; i < log_uv; ++i)
            {
                quadratic_poly2 poly = sumcheck_phase2_update(previous_random, i, (1 << (log_uv - i)));
                proof_size += sizeof(quadratic_poly2);
                ...
                alpha_beta_sum = poly.eval2(r_v[i]);
            }

            auto v_v = V_mult_add[0].eval2(previous_random);
...
int engage_gkr(int lg_size)
{
    ...
    prime_field::field_element a_0 = V_output(one_minus_r_0, r_0, C_struct.circuit_val[C_struct.circuit_val.size() - 1], mylog(C_struct.size[C_struct.size.size() - 1]), C_struct.size[C_struct.size.size() - 1]);
    ...
    std::chrono::high_resolution_clock::time_point t0 = std::chrono::high_resolution_clock::now();
    result &= addition_layer(alpha_beta_sum, C_struct.circuit_val[C_struct.circuit_val.size() - 2], C_struct.size[C_struct.size.size() - 2] / 64, 64, r_0, one_minus_r_0, r_1, one_minus_r_1, r_u, one_minus_r_u, r_v, one_minus_r_v);
    result &= mult_layer(alpha_beta_sum, C_struct.circuit_val[C_struct.circuit_val.size() - 3], C_struct.size[C_struct.size.size() - 3], 64, r_0, one_minus_r_0, r_1, one_minus_r_1, r_u, one_minus_r_u, r_v, one_minus_r_v);
    result &= intermediate_layer(alpha_beta_sum, C_struct.circuit_val[C_struct.circuit_val.size() - 4], C_struct.size[C_struct.size.size() - 4], r_0, one_minus_r_0, r_1, one_minus_r_1, r_u, one_minus_r_u, r_v, one_minus_r_v);
    //ifft part
    result &= ifft_gkr(alpha_beta_sum, C_struct.size[C_struct.size.size() - 4], r_0, one_minus_r_0, r_1, one_minus_r_1, r_u, one_minus_r_u, r_v, one_minus_r_v);
    //extension part
    result &= extension_gkr(alpha_beta_sum, C_struct.size[C_struct.size.size() - 4], r_0, one_minus_r_0, r_1, one_minus_r_1, r_u, one_minus_r_u, r_v, one_minus_r_v);
    std::chrono::high_resolution_clock::time_point t1 = std::chrono::high_resolution_clock::now();
