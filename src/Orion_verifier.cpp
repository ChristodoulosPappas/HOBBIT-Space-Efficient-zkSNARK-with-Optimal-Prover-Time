#include "Orion_verifier.h"
#include <string>
#include <utility>
#include <vector>
#include <iostream>
#include "random_generator.h"
#include "virgo_vpd_verifier.h"
...
/*
...

	{
		...
		C.circuit[1].gates = new _gate[witness_count_padded];
...
void zk_verifier::read_circuit(const char *path, const char *meta_path)
{
	...
	for(int i = 1; i <= d; ++i)
	{
		...
		{
			if(n == 1)
				C.circuit[i].gates = new _gate[2];
			else
				C.circuit[i].gates = new _gate[n_pad];
		}
		else
		{
			if(n == 1)
			{
				C.circuit[0].gates = new _gate[2];
				C.circuit[1].gates = new _gate[2];
			}
			else
			{
				C.circuit[0].gates = new _gate[n_pad];
				C.circuit[1].gates = new _gate[n_pad];
			}
		}
		...
		for(int j = 0; j < n; ++j)
		{
			...
			{
				if(v != 0)
					fprintf(stderr, "WARNING, v!=0 for NOT _gate.\n");
				v = 0;
			}
			if(ty == 10)
			{
				if(v != 0)
					fprintf(stderr, "WARNING, v!=0 for relay _gate. %d\n", i);
				v = 0;
			}
			if(ty == 13)
			{
			...
			previous_g = g;
			if(i != 1)
				C.circuit[i].gates[g] = _gate(ty, u, v);
			else
			{
				assert(ty == 2 || ty == 3);
				C.circuit[1].gates[g] = _gate(4, g, 0);
				C.circuit[0].gates[g] = _gate(ty, u, v);
			}
		}
        ...
        if(i == 1)
        {
            ...
            {
                C.circuit[1].gates[g] = _gate(4, g, 0);
                C.circuit[0].gates[g] = _gate(3, 0, 0);
            }
            n = n_pad;
            previous_g = n_pad - 1;
        }
		...
		if(n == 1)
		{
			//add a dummy _gate to avoid ill-defined layer.
			if(i != 1)
			{
				C.circuit[i].gates[max_gate] = _gate(2, 0, 0);
				C.circuit[i].bit_length = cnt;
			}
			else
			{
				C.circuit[0].gates[max_gate] = _gate(2, 0, 0);
				C.circuit[0].bit_length = cnt;
				C.circuit[1].gates[max_gate] = _gate(4, 1, 0);
...
bool zk_verifier::verify(const char* output_path)
{
	...
	for(int i = C.total_depth - 1; i >= 1; --i)
	{
		...
		for(int j = 0; j < C.circuit[i - 1].bit_length; ++j)
		{	
			quadratic_poly2 poly = p -> sumcheck_phase1_update(previous_random, j);
			...
			alpha_beta_sum = poly.eval2(r_u[j]);
		}
	...
	//	std::cerr << "Bound v start" << std::endl;
		...
		for(int j = 0; j < C.circuit[i - 1].bit_length; ++j)
		{
			if(i == 1)
				r_v[j] = prime_field::field_element(0);
			quadratic_poly2 poly = p -> sumcheck_phase2_update(previous_random, j);
			proof_size += sizeof(quadratic_poly2);
			poly.c = poly.c;
		
			previous_random = r_v[j];
			if(poly.eval2(0) + poly.eval2(1) + direct_relay_value * p -> v_u != alpha_beta_sum)
			{
			...
			alpha_beta_sum = poly.eval2(r_v[j]) + direct_relay_value * p -> v_u;
		}
	...
	else
	{
		...
		//std::cerr << "Proof size(bytes) " << proof_size << std::endl;
 		Elastic_PC_vt += verification_time/100;
		Elastic_PC_ps +=  (double)proof_size/1024.0;
		Elastic_PC_pt += p -> total_time;
