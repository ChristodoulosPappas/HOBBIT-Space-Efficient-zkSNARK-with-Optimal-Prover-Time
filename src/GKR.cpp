...
void subsetInit() {
    ...
    for (int i = 0; i < C.size; ++i) {
        //Changed
        visited_idx[i].resize(_count_size(C.circuit[i]));
        subset_idx[i].resize(_count_size(C.circuit[i]));
    }
...
int parse_test_circuit(ifstream &circuit_in, long long int *in, int N, int layers);
int parse_test_circuit2(ifstream &circuit_in, long long int *in, int N, int layers);
void parse_m2m(ifstream &circuit_in, long long int *in, int col_size,int row_size, int layers);
int parse_sha256(ifstream &circuit_in,long long int *in, int instances);
...
int test_sha256(vector<F> data, vector<F> randomness, int hashes){
    ...
    int m_depth = parse_sha256(circuit_in, in, 0);

    C = DAG_to_layered();
    
    subsetInit();
    for(int i = 0; i < C.size; i++){
        if((1ULL << C.circuit[i].bitLength) > m_depth){
            m_depth = 1ULL << C.circuit[i].bitLength;
            printf("%d\n",m_depth);
        }
    }
    return m_depth*hashes;
}
...
int test_multree(vector<F> data, vector<F> randomness, int N, int M, int layers){
    ...
    ifstream circuit_in;
    in_circuit_dag.clear();
    int m_depth = parse_test_circuit(circuit_in, in, N, layers);
...
int test_gkr_standard(int N, int M, int layers){
    ...
    ifstream circuit_in;
    in_circuit_dag.clear();
    int m_depth = parse_test_circuit2(circuit_in, in, N, layers);
