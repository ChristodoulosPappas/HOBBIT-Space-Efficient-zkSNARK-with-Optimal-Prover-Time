...
namespace poly_commit
{
	...
	public:
		...
		__hhash_digest commit_private_array(prime_field::field_element *private_array, int log_array_length)
		{
			...
			{
				prime_field::field_element *tmp,*l_tmp;
				tmp = new prime_field::field_element[slice_real_ele_cnt];
				l_tmp = new prime_field::field_element[slice_size];
				...
				else
				{
					...
					fast_fourier_transform(tmp, slice_real_ele_cnt, slice_size, prime_field::get_root_of_unity(mylog(slice_size)), l_tmp);
					for(int j = 0; j < slice_size; j++){
						l_eval[i*slice_size + j] = l_tmp[j];
					}
				}
				delete[] tmp;
				delete[] l_tmp;
