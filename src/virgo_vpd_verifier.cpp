...
{	
    ...
    float temp_commit_timer = commit_timer;
    commit_timer = 0.0;
    clock_t t1,t2;
    t1 = clock();
    ...
    //printf("FRI time : %lf,%lf\n",(double)(t2-t1)/(double)CLOCKS_PER_SEC + 11.0*commit_timer,(double)(t2-t1)/(double)CLOCKS_PER_SEC-commit_timer );
    commit_timer = temp_commit_timer+commit_timer;
    ...
    for(int rep = 0; rep < 33; ++rep)
    {
        ...
        for(int i = 0; i < log_length - log_slice_number; ++i)
        {
            ...
            if(i == 0)
            {
                ...
                proof_size += alpha_l.second.size()*sizeof(__hhash_digest);
                proof_size += alpha_h.second.size()*sizeof(__hhash_digest);
                t0 = std::chrono::high_resolution_clock::now();
                ...
                proof_size +=  beta.second.size()*sizeof(__hhash_digest);
                proof_size += new_size;
                ...
                for(int j = 0; j < slice_count; ++j)
                {
                    ...
                    {
                        //fprintf(stdout, "Fri check consistency first round fail %d\n", j);
                        //return false;
                    }
            ...
            else
            {
               ...
               // std::cerr << "Verification Time " << v_time << std::endl;
                ...
                for(int j = 0; j < slice_count; ++j)
                {
                    ...
                    {
                        //fprintf(stdout, "Fri check consistency %d round fail\n", i);
                        //return false;
                    }
                }
            }
        ...
        for(int i = 0; i < slice_count; ++i)
        {
            ...
            for(int j = 0; j < (1 << (rs_code_rate - 1)); ++j)
            {
                ...
                {
                    //fprintf(stdout, "Fri rs code check fail\n");
                    //return false;
                }
            }
        }
