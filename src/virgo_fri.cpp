...
__hhash_digest fri::request_init_commit(const int bit_len, const int oracle_indicator)
{
	...
	*/	
	...
	clock_t _t1,_t2;
	_t1 = clock();
	//printf("Size: %d\n",(int)log2((1 << (log_current_witness_size_per_slice - 1))*(1 << (log_leaf_size))));
	...
	merkle_tree_virgo::merkle_tree_prover::create_tree(leaf_hash[oracle_indicator], 1 << (log_current_witness_size_per_slice - 1), witness_merkle[oracle_indicator], sizeof(__hhash_digest), true);
	_t2 = clock();
	commit_timer += (double)(_t2-_t1)/(double)(CLOCKS_PER_SEC);
	//printf("MT generation time : %lf \n",12*commit_timer);
...
__hhash_digest fri::commit_phase_step(prime_field::field_element r)
{
	...
	merkle_tree_virgo::merkle_tree_prover::create_tree(hash_val, nxt_witness_size / 2, cpd.merkle[current_step_no], sizeof(__hhash_digest), cpd.merkle[current_step_no] == NULL);
	_t2 = clock();
